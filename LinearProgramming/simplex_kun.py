# -*- coding: utf-8 -*-
"""
Created on Wed Sep  9 15:30:21 2020
...
# check if the last row has positive index 
...
if __name__ == "__main__":
    ...
    # c = [ -x for x in c] # it is for maximize

    # add slack variables  
    Ap  = np.array(A)
    Aslack = np.eye(row, dtype = float)
    A = np.hstack((Ap, Aslack))
    A = A.tolist()
    c += [0]*row
