...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  for (Node* tempPtr = getHeadPtr(); tempPtr != nullptr; tempPtr = tempPtr->next) {
      
    if (tempPtr == getTailPtr() && tempPtr->data < newData) {
      // to end
      pushBack(newData);
      return;
    }
    
    if ( newData < tempPtr->data ) {
      if (tempPtr == getHeadPtr()) {
